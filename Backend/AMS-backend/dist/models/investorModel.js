...
exports.findInvestor = findInvestor;
exports.checkInvestorExists = checkInvestorExists;
...
async function findInvestor(mobile) {
    // const data = investors.find((inv) => inv.mobile == mobile);
    const getInvPromise = new Promise((resolve, reject) => {
        db_1.database.get(`SELECT * from investor where investor_id = '${mobile}'; `, (error, data) => {
            if (error)
                reject(error);
            resolve(data);
        });
...
function checkInvestorExists(mobile) {
    const data = investors.find((inv) => inv.mobile == mobile);
    if (data == undefined) {
        return false;
    }
