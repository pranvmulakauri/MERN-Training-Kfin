...
export const logoutUser = (email, token) => {
...
function getInvestorFromDB(mobile) {
    return new Promise((resolve, reject) => {
        client.query(`select * from investor where pancard_no = '${mobile}';                `, (err, rows) => {
            if (err) {
                console.log(err);
                reject(err);
            }
...
function calculateNavfromDB(mobile) {
    return new Promise((resolve, reject) => {
        db.get(`
...
export async function fetchInvestorData(mobile) {
    try {
        const investor = await getInvestorFromDB(mobile);
        return investor;
    }
    catch (e) {
        console.error(e);
...
export async function calculateNAV(mobile) {
